...
const Footer = () => (
  ...
  flex-col`}>
      ...
      justify-between flex-wrap md:mt-0 mt-10'>
        {footerLinks.map((footerLink) => (
          <div key={footerLink.key} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
            <h4 className='font-poppints font-medium text-[18px]
            leading-[27px] text-white'>
              {footerLink.title}
            </h4>
            <ul className='list-none mt-4'>
              {footerLink.links.map((link, index) => (
                <li key={link.name} className={`font-poppints
    ...
    md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3e45]'>
      <p className='font-poppints font-normal text-center text-[18px]
            leading-[27px] text-white'>
        2024 HooBank. All Rights Reverved.
      </p>
